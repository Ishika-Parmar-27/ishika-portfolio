      ...
      <div className="max-w-6xl mx-auto px-6 grid lg:grid-cols-2 gap-20 items-center">
        ...
        >
          ...
          <p className="text-slate-700 leading-8 text-lg">
            ...
            <span className="font-semibold text-[#566b1f]">
              Java Full Stack Intern
            </span>
            , where I'm gaining practical experience in Spring Boot, React,
...
function Card({ icon, title, text }) {
  return (
    <div className="bg-[#fafaf9] border border-gray-200 shadow-sm rounded-2xl p-7 hover:-translate-y-2 hover:shadow-xl transition-all duration-300">
      <div className="text-[#6d812d] mb-4">{icon}</div>
      <h3 className="font-bold text-2xl mb-3">{title}</h3>
      <p className="text-slate-700 leading-6">{text}</p>
    </div>
  );
}
